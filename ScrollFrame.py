...
class ScrollFrame(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.canvas.configure(
            yscrollcommand=self.vsb.set)  # attach scrollbar action to scroll of canvas
        ...
        self.viewPort.bind('<Enter>',
                           self.onEnter)  # bind wheel events when the cursor enters the control
        self.viewPort.bind('<Leave>',
                           self.onLeave)  # unbind wheel events when the cursorl leaves the control
    ...
    def __init__(self, root):
        ...
        for row in range(100):
            ...
            tk.Button(self.scrollFrame.viewPort, text=t,
                      command=lambda x=a: self.printMsg("Hello " + str(x))).grid(row=row, column=1)
